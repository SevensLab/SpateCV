...
if isinstance(adata_seq.X, sp.csr_matrix):
    adata_seq.X = adata_seq.X.toarray()# Convert the sparse matrix to a dense matrix
...
sc_data = pd.DataFrame(data_seq_array,columns=sp_genes)

sc_data.columns = sc_data.columns.astype(str)
sp_data.columns = sp_data.columns.astype(str)
...
def uniport_impute():
    global sc_data, sp_data, adata_seq, adata_spatial
    adata_spatial.obs['domain_id'] = 0
    adata_seq.obs['domain_id'] = 1
    ...
    for train_ind, test_ind in kf.split(raw_shared_gene):
        ...
        adata = up.Run(adatas=adatas, adata_cm=adata_cm, lambda_kl=5.0, model_info=False,gpu=0)
...
def CalDataMetric(PATH,Data):
    ...
    if len(impute_count)!=0:
        ...
        for impute_count_file in impute_count:
            ...
            CM = CalculateMetrics(data_spatial_array, sp_genes, impute_count_file = impute_count_file, prefix = prefix, metric = metric)
            CM.compute_all()
            ...
            for j in ['_gene']:
                ...
                for m in metric:
                    median.append(np.nanmedian(tmp[m].values))
            ...
            # Calculate the mean ± std of gene - level metrics
            means, stds = [], []
            for j in ['_gene']:
                tmp = pd.read_csv(prefix + j + '_Metrics.txt', sep='\t', index_col=0)
                for m in metric:
                    means.append(np.nanmedian(tmp[m].values))
                    stds.append(np.nanstd(tmp[m].values, ddof=1))

            fmt_gene = [f"{mu:.4f}±{sigma:.4f}" for mu, sigma in zip(means, stds)]
            gene_df = pd.DataFrame([fmt_gene], columns=metrics)

            clu = pd.read_csv(prefix + '_cluster' + '_Metrics.txt', sep='\t', index_col=0)
            cluster_fmt = [f"{clu[col].values[0]:.4f}±0.0000" for col in ["ARI","AMI","Homo","NMI"]]
            cluster_df = pd.DataFrame([cluster_fmt], columns=["ARI","AMI","Homo","NMI"])

            result = pd.concat([gene_df, cluster_df], axis=1)
            results = pd.concat([results,result], axis=0)

        metrics += ["ARI", "AMI", "Homo", "NMI"]
        medians.columns = metrics
        medians.index = methods
        medians.to_csv(outdir +  './final/result.csv',header = 1, index = 1)
        
        results.columns = metrics
        results.index = methods
        results.to_csv(outdir +  './final/result_std.csv',encoding='utf-8-sig',header = 1, index = 1)
